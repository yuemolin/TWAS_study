...
samples=$4
for ((i=1; i<=22; i++)); do
	mkdir "${out_path}/chr${i}"
	vcfFile=${vcfPath}/chr${i}.combined.poly.dose.vcf.gz
	# transform vcf file to bcf file and filter MAF and R2 then output
	bcftools view -O b -S $samples -v snps -i "MAF>0.01 & R2>0.8" $vcfFile -o "$out_path/chr${i}/temp.bcf.gz"
	# creat an index file for code below
	bcftools index -c "$out_path/chr${i}/temp.bcf.gz"
	...
	{	
		...
		do
			...
			if [ $chr == $i ];then
				...
				"bcftools view -r "$chr:$lft_range-$rt_range" "$out_path/chr${i}/temp.bcf.gz" | \
				tee >(bcftools query -f '%ID, %ALT{}, %REF, [%DS, ]\n' | \
		       		gzip  > "$out_path/chr${i}/$geneID.bimbam.gz") | bcftools query -f '%ID, %POS, %CHROM\n' | \
				gzip > "$out_path/chr${i}/$geneID.annot.gz"" 
			fi
		done
	} < $geneFile
	echo "chr${i}finished"
